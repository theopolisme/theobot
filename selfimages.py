	...
	if bot.donenow("User:Theo's Little Bot/disable/selfimages",donenow=donenow,donenow_div=5,shutdown=100) == True:
		...
		if bot.nobots(usertalktitle,user="Theo's Litle Bot",task='selfimages') == True:
			...
			if original_talk.find('<!--Template:Un-botfill-->') == -1:
				...
				talkcontents = re.sub(r"""Template:Un-botfill-->(.*?)(?:\n|\r|\Z)""","""Template:Un-botfill-null-->\g<1>\n\n*{{subst:Un-botfill/followup|file="""+page.page_title+"|sig=~~~~}}\n\n",original_talk,flags=re.UNICODE)
			usertalk.save(talkcontents,"[[WP:BOT|Bot]]: Notifying user about autogenerated {{[[Template:Information|Information]]}} addition) ([[User:Theo's Little Bot/disable/selfimages|disable]]",redirect=True)
	else:
		sys.exit()
