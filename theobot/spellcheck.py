...
"""Misspellings module.
	Take a string, check it against a list of misspelled words.
	Based on https://github.com/lyda/misspell-check/blob/master/misspellings_lib.py
"""
...
class Misspellings(object):
	"""Detects misspelled words in a string."""

	def __init__(self, stringtocorrect=None):
		...
		"""
		self.stringtocorrect = stringtocorrect

	def check(self):
		"""Checks the string for misspellings.

		Returns:
			(results)
			results: List of spelling errors - each tuple is line number and misspelled word.
		"""
		results = []
		line_ct = 1
		for line in self.stringtocorrect.splitlines():
			for word in split_words(line):
				if (word in self._misspelling_dict or
						word.lower() in self._misspelling_dict):
					results.append([line_ct, word])
			line_ct += 1
		return results
