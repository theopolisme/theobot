	...
	if already_told(user) == False:
		if bot.nobots(page=usertalk,task='bsr') == True:
			...
			if notified_already(user) == True:
				...
				try:
					page.save(appendtext=text,summary="Notifying user about file(s) with inadequate source information ([[WP:BOT|bot]] - [[User:Theo's Little Bot/disable/bsr|disable]])",redirect=True)
