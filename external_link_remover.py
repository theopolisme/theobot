...
try:
	...
	summary = "Removing links to defunct sites: {0}. ([[WP:BOT|bot]] on trial - [[User:Theo's Little Bot/disable/external links|disable]])".format(", ".join(naughty_links))
except IndexError:
	summary = "Removing links to defunct site, {0}. ([[WP:BOT|bot]] on trial - [[User:Theo's Little Bot/disable/external links|disable]])".format(naughty_links[0])
...
for page in full_results:
	if page[u'ns'] == 0 and page[u'title'] not in to_process and done < 11:
...
for page in to_process:
	if donenow("User:Theo's Little Bot/disable/external links",donenow_div=5) == True:
		print "~~~\n~~~\n"
		contents = page.edit()
		for link in naughty_links:
		...
		sys.exit('Program was disabled on-wiki.')
